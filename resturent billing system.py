...
def update_order():
    # Updates the order quantities if an item is already selected
    try:
        for item, entry in {
            "roti": e_roti,
            "daal": e_daal,
            ...
            "jaljeera": e_jaljeera,
            "roohafza": e_roohafza,
            "masalatea": e_masalatea,
            "badammilk": e_badammilk,
            "colddrinks": e_coldrinks,
        }.items():
            if entry.get() != "0":
                entry.set(str(int(entry.get()) + 1))  # Add 1 to the existing quantity
            
    except ValueError:
        messagebox.showerror("Error", "Invalid quantity input. Please enter valid numbers.")
...
labelTitle=Label(topFrame,text='Restaurant Billing System',font=('arial',30,'bold'),fg='yellow',bd=9,
                 bg='red4',width=51)
...
buttonFrame=Frame(rightFrame,bd=4,relief=RIDGE,bg='red4')
buttonFrame.pack()
...
buttonUpdate = Button(buttonFrame, text="Update", font=("arial", 14, "bold"), fg="white", bg="red4", bd=3, padx=5,
                      command=update_order)
buttonUpdate.grid(row=0, column=3)  # Positioned next to Save
